...
\begin{document}
	...
	\section{Sorting}
		...
		\subsection{Heap Sort} \lecture{January 24, 2013}
			...
			\begin{proof} We're going to show that we can run this algorithm in $\Theta(n)$ time. \\
				...
				\begin{align*}
					\frac{n}{2} + 1& + \frac{n}{4} \cdot 2 + \frac{n}{8} \cdot 3 + \ldots + 1 \cdot \lg n \\
	...
	\section{More Abstract Data Types}
		\subsection{Dictionaries}
			...
			\subsubsection{Supported Operations of Dictionaries}
			...
			\subsubsection{Implementations of Dictionaries}
				...
				\lecture{February 7, 2013}
				\subsubsection{Binary Search Trees and AVL Trees}
					...
					\begin{figure}[H]
						\Tree [.o [.o [.o [.o ] [.o ] [.o ]] [.o ]] !{\qframesubtree} [.o [.o ]] !{\qframesubtree} ]
					...
					\\ \\
					\textbf{Right rotation}: if the balance of a particular subtree is -2, then a right rotation should be applied to that subtree.
					\begin{figure}[H]
						\qroofx=2
						\qroofy=2
					...
					\\ \\
					\textbf{Left rotation}: if the balance of a particular subtree is +2, then a left rotation should be applied to that subtree.
					\begin{figure}[H]
						\qroofx=2
						\qroofy=2
					...
					\textbf{Double-right rotation}: if the inner-left side is heavy (and the balance for the tree is -2), a double-right rotation should be applied to that subtree. A normal right rotation does not property rebalance the tree.
					\begin{figure}[H]
						\qroofx=2
						\qroofy=2
					...
					\\ \\
					\textbf{Double-left rotation}: if the inner-right side is heavy (and the balance for the tree is +2), a double-left rotation should be applied to that subtree. A normal left rotation does not properly rebalance the tree.
					\begin{figure}[H]
						\qroofx=2
						\qroofy=2
						\Tree [.z \qroof{A}. [.y [ \qroof{X}. \qroof{B}. \qroof{C}. ].X \qroof{D}. ].y ].z
				...
				\subsubsection{Handling RAM Constraints}
			...
			\subsubsection{Comparison of Trees} \lecture{February 14, 2013} % Today's notes were taken from Kevin Cho's handwritten notes (thanks!).
				...
				In a B-tree of height $h$, what is the minimum number of nodes in the tree? $2 + 2 \cdot 2 + 2 \cdot 2 \cdot 2 + \ldots$, since the root has two children, and each of those has two children, and so on. This leaves us with the minimum number of nodes in the tree being:
				\begin{align*}
					2 + 2 \cdot 2 + 2 \cdot 2 \cdot 2 + \ldots &= \sum_{i = 1}^{h} \bigg(\frac{m}{2}\bigg)^i \approx \bigg(\frac{m}{2}\bigg)^{h + 1} \\
				...
				In a B-tree of height $h$, what is the maximum number of nodes in the tree? $4 + 4 \cdot 4 + 4 \cdot 4 \cdot 4 + \ldots$, following the same logic as before. This leaves us with:
				\begin{align*}
					4 + 4 \cdot 4 + 4 \cdot 4 \cdot 4 + \ldots &= \sum_{i = 0}^{h} m^i \approx m^{h + 1} \\
					m^{h + 1} &= n \\
					(h + 1) \lg m &= \lg n \\
