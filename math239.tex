...
;					\end{tikzpicture}
			...
			\begin{ex}
				...
				\begin{center}
					\begin{tikzpicture}[scale=1.5]
						...
						\draw (0,0) -- (3,0) -- (3,2) -- (0,2) -- (0,0);
						\draw (0,1) -- (3,1);
						\draw (0,0) to[out=150,in=210] (0,2);
						\draw (3,0) to[out=30,in=-30] (3,2);
	...
	\section{Matching}
		...
		Our goal for the remainder of the course: we want to find an algorithm to produce a matching with as many edges as possible. That's a tough goal, so well restrict ourselves to finding a maximum matching in bipartite graphs only.
		\\ \\
		...
		\subsection{Application: co-op}
			...
			\subsection{Saturation}
				...
				\begin{defn}
					Let $G$ be a graph with matching $M$. Vertex $v$ is \textbf{saturated} if $v$ is the endpoint of an edge of $M$.
				\end{defn}
		...
		\subsection{XY-Structures}
			...
			\begin{ex} \lecture{April 5, 2013}
				...
				We have that $X = \set{1, 2}$ and $Y = {5}$. $Y$ contains only saturated vertices, so we stop. The cover we found is: $$C = (A \backslash X) \cup Y = \set{1, 2, 3, 4} \backslash \set{1, 2} \cup \set{5} = \set{3, 4, 5}$$
			...
			\begin{ex}
				...
				This is a bipartite graph with partitions $A = \set{1, 2, 3, 4}$ (red) and $B = {5, 6, 7, 8}$ (green).
				\\ \\
		...
		\subsection{Hall's Theorem}
			...
			\\ \\
			We need at least the same number of vertices in $N(S)$ (the neighbors of $S$), otherwise a perfect matching is not possible.
			
			\begin{defn}
				Let $G = (V, E)$ be a bipartite graph with partitions $A$ and $B$. Let $S \subseteq A$. Then $N(S) = \set{v \in V | \exists uv \in E, u \in S}$. That is, $N(S)$ denotes the neighbors of $S$.
			\end{defn}
			...
			\begin{proof}
				We proved the forwards direction of this statement in the previous remark.
				\\ \\
				For the reverse direction, assume $|A| = |B|$ and there does not exist a perfect matching. We need to show that there exists $S \subseteq A$ such that $|N(S)| < S$ (that is, we're aiming to prove the contrapositive).
				\\ \\
				By K\"onig's Theorem, there exists a cover $C$ of size strictly less than $|A|$.
				\\ \\
				(We'll finish this proof next time.)
			\end{proof}
	...
	\section*{Clicker Questions}
		\begin{itemize}
			...
			\item What is the minimum number of colors needed to color this graph (not shown here)? Four, because this was contained within the graph:		
				\begin{center}
					\begin{tikzpicture}[scale=2]
						\node [dot={0}{}] at (0,0) {};
						\node [dot={0}{}] at (1,1) {};
