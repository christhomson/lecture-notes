...
\begin{document}
			...
			\subsubsection{Permutations}
				...
				\subsubsection{Application: Binomial Theorem} \lecture{January 9, 2013}
					...
					\begin{proof}
						...
						In order to get $x^k$, we need to choose $x$ in $k$ of $\{1, \ldots, n \}$. There are $\binom{n}{k}$ ways of doing this.
					\end{proof}
	...
	\section{Binary Strings}
		...
		\begin{defn}
			The \textbf{concatenation $\mathbf{AB}$ of two sets} ($A$ and $B$) of binary strings is defined as all possible strings formed as concatenations of one string from set $A$ followed by one string from set $B$. For example, if $A = \{ 011, 11 \}$ and $B = \{10, 0\}$, then $AB = \{ 01110, 0110, 1110, 110 \}$.
		\end{defn}
	...
	\section{Introduction to Graph Theory} \lecture{February 15, 2013} % Today's notes were taken from Kevin Cho's handwritten notes (thanks!). 
		...
		\subsection{Walks, Cycles, and Paths}
			...
			\begin{proof}
				...
				Since $\deg(V_n) \ge 2$, there exists $V_nw \ne e_n$. Since $P$ is the longest path, $w = V_i$ for some $i \in \{0, \ldots, n - 2\}$. Then we have the cycle $C$:
				\begin{align*}
...
;					\end{tikzpicture}
			...
			\subsubsection{Bridges}
				...
				\begin{proof}
					...
					Using those facts, by transitivity we know there exists a path $P$ between $x$ and $y$ in $G \backslash e$. Then $P \cup \{e\}$ is a cycle.
				\end{proof}
		\subsection{Trees}
			\begin{defn}
			...
			\begin{proof}
				$C$ is a cycle of $G$ if and only if $C \backslash \{e\}$ is a path of $H$ between $a$ and $b$. We know that there exists a unique such path.
			\end{proof}
