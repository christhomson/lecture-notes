...
\begin{document}
	...
	\section{Outline of an Assembler} \lecture{January 23, 2013}
		...
		\subsection{Pass 1 \textendash{ } Analysis}
			...
			The supplied scanners will find a sequence of tokens on the given line for you. For each token, you'll get a tuple (kind, lexene, value), where the lexene is the literal text of the token. For example, take the line \verb+foo: beq $3, $6, -2\verb+. The provided scanners will return a list with this data:
			\begin{itemize}
				\item (LABEL, ``for:'', ?)
				\item (OPCODE, ``beq'', ?)
				\item (REGISTER, ``\$3'', 3)
