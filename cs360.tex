  ...
  \subsection{Undecidability: The Halting Problem}
    ...
    \subsubsection{Recognizability of the Halting Problem} \lecture{November 16, 2013}
      ...
      \begin{proof}
        ...
        \\ \\
        This argument \emph{relies} on $M$ halting on every input, because for $\overline{M}$ to accept or reject some input, it must simulate $M$, wait for accept/reject, and then take the opposite action.
        \\ \\
    ...
    \subsection{Proving Undecidability: The Reduction Technique}
      ...
      \begin{ex}
        ...
        \\ \\
        \textbf{Claim}: $E_\text{mpt}$ is not decidable. This is intuitive when you consider a question like ``will I ever fall in love?'', and we define a language of all people you will fall in love with. But clearly, this question is not easily answered.
      ...
      \begin{ex}
        Let $EQ = \set{(\langle M_1 \rangle, \langle M_2 \rangle) : L(M_1) = L(M_2)}$.
        \\ \\
        \textbf{Claim}: $EQ$ is not decidable. Note that $\overline{EQ}$ is recognizable, since we can just start running the machines on various inputs, then the moment they disagree on some input, we can instantly say that they are not equivalent.

        \begin{proof}
          We will reduce the undecidable emptiness problem to $EQ$. Recall that $E_\text{mpt}$ takes one machine, $M$, as input and accepts if the language is empty, or rejects otherwise.
          \\ \\
          Pick some simple $M_0$ that rejects every input. $(\langle M \rangle, \langle M_0 \rangle)$ as input into $EQ$. If $EQ$ accepts, then $L(M)$ is empty, so we should accept. Otherwise, if $EQ$ rejects, then $L(M)$ is not empty, so we should reject.
          \\ \\
          We know $E_\text{mpt}$ is undecidable, so this is a contradiction. Therefore, $EQ$ is not decidable.
        \end{proof}
